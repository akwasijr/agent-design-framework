...
y = body(
    "Just as we map user journeys today, we need a way to map agent journeys. Agent journeys are "
...
txt("Approaches for Mapping Agent Journeys", M, y, sz=16, font="Helvetica-Bold")
y -= 10
accent_line(M, y, 40)
y -= 18
...
approaches = [
    ("Agent Journey Canvas",
     "A structured canvas with rows for human intent, agent interpretation, agent plan, approval points, "
     "execution, output and feedback loop. Take a workflow you know well, map how it works today, then "
     "remap it as an agent journey to see where approvals sit and what can run silently."),
    ("Trust Level Stacking",
     "Take one workflow and map it at four trust levels: new (everything approved), building (routine auto, "
     "major approved), established (only exceptions surface), trusted (summary only). This reveals how the "
     "UX collapses over time and where the earned autonomy moments are."),
    ("Exception Storming",
     "Generate as many \"what could go wrong\" scenarios as possible for a given agent workflow, then "
     "categorise them: judgment, authority, ethics, novelty, conflict, low confidence. Each category implies "
     "a different interface pattern. Since the happy path runs silently, the exceptions are the UX."),
    ("Handoff Mapping",
     "For multi-agent workflows, map every handoff between agents. At each: what information passes, what "
     "could be lost, where agents might conflict, where a human needs visibility. Handoff points are where "
     "choreography UX gets designed."),
    ("Day in the Life Storyboard",
     "Storyboard a full workday in a specific domain: morning briefing, first decisions, midday exception, "
     "afternoon review, end of day summary. Forces concrete, specific thinking rather than abstract "
     "descriptions of what agents could do."),
]
